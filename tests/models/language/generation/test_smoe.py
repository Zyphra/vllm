...
HYBRID_MODELS = [
    "Zyphra/smoe-midtrain_phase2_decay-30036",
]

# Avoid OOM
MAX_NUM_SEQS = 4

@pytest.mark.skip(reason="We're facing issues with getting SMOE running due to the Gemma3Tokenizer having a vision component. The checkpoint itself doesn't have vision support.")
...
def test_chunked_prefill_with_parallel_sampling(
    ...
    In this case, prefill is populated with decoding tokens and we test that
    it doesn't fail. This test might fail if cache is not allocated correctly
    for n > 1 decoding steps inside a chunked prefill forward pass (where we
    have both prefill and decode together)
    """
    sampling_params = SamplingParams(n=3, temperature=1, seed=0, max_tokens=max_tokens)
    with vllm_runner(
        model,
        enable_chunked_prefill=True,  # forces prefill chunks with decoding
...
def test_mamba_cache_cg_padding(
    ...
    """
    This test is for verifying that mamba cache is padded to CG captured batch
    size. If it's not, a torch RuntimeError will be raised because tensor
    dimensions aren't compatible.
    """
...
def test_fail_upon_inc_requests_and_finished_requests_lt_available_blocks(
    ...
    statelessness mechanism where it can clean up new incoming requests in a
    single step.
    """
    try:
        with vllm_runner(model, max_num_seqs=MAX_NUM_SEQS) as vllm_model:
...
def test_state_cleanup(
    ...
    steps. If it's not cleaned, an error would be expected.
    """
    try:
        with vllm_runner(model, max_num_seqs=MAX_NUM_SEQS) as vllm_model:
            for _ in range(10):
...
def test_apc_single_prompt(
    ...
    compare_operator: Callable = (
        check_logprobs_close
        if num_logprobs > 0
        else check_outputs_equal  # type: ignore
    )
...
def test_apc_multiple_prompts_block_align_alignment(
    ...
    vllm_runner_kwargs = _get_vllm_runner_params(model, max_model_len, tensor_parallel_size)
    vllm_runner_kwargs["mamba_ssm_cache_dtype"] = "float32"
