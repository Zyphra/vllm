...
Start a normal OpenAI-compatible backend, for example:
...
def pop_rsa_config(body: dict[str, Any]) -> tuple[RSAConfig | None, dict[str, Any]]:
    ...
    rsa_n_default = upstream_body.get("n", 16)
    config = RSAConfig(
        rsa_t=int(_pop_alias(upstream_body, "rsa_t", 0)),
        rsa_k=int(_pop_alias(upstream_body, "rsa_k", 4)),
        ...
        tail_window_len=int(_pop_alias(upstream_body, "tail_window_len", 4096)),
        short_resp_len=int(_pop_alias(upstream_body, "short_resp_len", 8192)),
...
def _merge_chunk_delta(choice_state: dict[str, Any], delta: dict[str, Any]) -> None:
    ...
    if delta.get("reasoning_content"):
        message["reasoning_content"] = (
            message.get("reasoning_content", "") + str(delta["reasoning_content"])
        )
...
def build_app(proxy: RSAProxy) -> FastAPI:
    ...
    async def create_chat_completion(raw_request: Request) -> Response:
        ...
        result = await proxy.run_rsa_chat_completion(
            upstream_body,
            config,
            raw_request,
        )
