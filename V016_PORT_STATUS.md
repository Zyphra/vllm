...
| | what | impact |
|---|---|---|
| **PIECEWISE only** | `CCAAttentionBackend` in v0.16 declares `_cudagraph_support = UNIFORM_SINGLE_TOKEN_DECODE` (inherited from `BaseMambaAttentionMetadataBuilder`), which causes the downgrade check at `gpu_model_runner.py:~5668` to force PIECEWISE for spec-decode batches. **Tried bumping** to `UNIFORM_BATCH` + adding a `build_for_cudagraph_capture` override on `CCAAttentionMetadataBuilder` — but those uncovered a deeper blocker: `vllm/model_executor/layers/mamba/cca.py:forward_triton`'s prefill path has a **Python loop** over requests that slices `hs_p[start_i:end_i]` with int indices, which trips `cudaErrorStreamCaptureUnsupported` under cudagraph capture. | Limits captured perf vs the v0.15.x handoff's 302 tok/s (FULL_DECODE_ONLY). Current best: 72 tok/s. To get FULL working, port the TiDAR-specific vectorized spec-decode methods from `jinzhao/tidar`'s `vllm/model_executor/layers/mamba/cca.py`: `_spec_decode_prefill_vectorized` (~130 lines, replaces the per-req loop with a vectorized GPU op), `_spec_decode_proposal_sub_loop` (~85 lines, SF proposal sub-segments), `commit_spec_decode_state` (~115 lines), plus routing logic to dispatch uniform-K+1 batches to the vectorized path. ~360 lines total. **Phase 3 work, ~3-5h.** |
