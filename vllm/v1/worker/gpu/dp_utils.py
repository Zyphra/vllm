...
def get_batch_metadata_across_dp(
    ...
    tensor[2][dp_rank] = tidar_draft_tokens
...
def get_cudagraph_and_dp_padding(
    ...
    dp_rank: int,
    tidar_draft_tokens: int = 0,
) -> tuple[bool, int, torch.Tensor | None, torch.Tensor | None]:
    if dp_size == 1:
        if cudagraph_size is not None:
            return True, cudagraph_size, None, None
        else:
            return False, num_tokens, None, None

    if num_tokens == 0:
        cudagraph_size = 0
    elif cudagraph_size is None:
        cudagraph_size = -1
    (num_tokens_across_dp, cudagraph_size_across_dp,
     tidar_draft_tokens_across_dp) = get_batch_metadata_across_dp(
         num_tokens,
         cudagraph_size,
         dp_size,
         dp_rank,
    ...
    if torch.all(num_tokens_across_dp == 0).item():
        ...
        return False, 0, None, tidar_draft_tokens_across_dp
