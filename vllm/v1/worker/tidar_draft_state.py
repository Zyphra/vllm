...
def stash_tidar_draft_state(
    ...
    if num_spec_tokens is None:
        discard_tidar_draft_state(probs_by_req_id, logits_by_req_id, req_ids)
        return
...
def take_tidar_draft_state(
    ...
    *,
    ...
    if require_probs and missing_probs:
        raise RuntimeError(
            "Stochastic TiDAR requires proposal probabilities for every "
            "draft-bearing scheduled request, but q state is missing or "
            f"short for request IDs {missing_probs}. Refusing the "
            "NO_DRAFT_PROBS/Dirac fallback because it would compute an "
            "incorrect rejection-sampling ratio."
        )
    ...
    return draft_probs, draft_logits
