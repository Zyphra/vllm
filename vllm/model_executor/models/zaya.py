...
class ZayaRouter(nn.Module):
    def __init__(
        ...
        # Resolve the route once, before compilation, so the custom-op path is
        # present in FULL graphs instead of being specialized away.
    ...
    def forward(
        ...
        if self._fused_router_ok and router_hidden_states.dtype == torch.bfloat16:
            use_eda = bool(
                self.use_eda and prev_router_hidden_states is not None)
            previous = (
